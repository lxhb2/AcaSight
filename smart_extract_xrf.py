...
for group_name in ['Z1', 'Z2', 'Z3', 'Z4', 'Z5', 'Z6', 'Z7']:
    ...
    for test_num in [1, 2, 3]:
        ...
        if cu is None or s is None:
            # 尝试匹配更宽松的模式
            # 例如: "Cu: 1.26%" 或 "Cu 1.26" 或 "Cu：1.26%"
...
for group_name in ['Z1', 'Z2', 'Z3', 'Z4', 'Z5', 'Z6', 'Z7']:
    ...
    print(f'{group_name}: Cu平均={cu_avg:.2f}% ({len(cu_values)}/3有效), S平均={s_avg:.2f}% ({len(s_values)}/3有效)')
    
    summary.append({
        '编号': group_name,
        '铜测试1(%)': tests[0]['cu'] if len(tests) > 0 and tests[0]['cu'] else '缺失',
        '铜测试2(%)': tests[1]['cu'] if len(tests) > 1 and tests[1]['cu'] else '缺失',
        '铜测试3(%)': tests[2]['cu'] if len(tests) > 2 and tests[2]['cu'] else '缺失',
        '铜平均(%)': round(cu_avg, 2) if cu_avg else '缺失',
        '硫测试1(%)': tests[0]['s'] if len(tests) > 0 and tests[0]['s'] else '缺失',
        '硫测试2(%)': tests[1]['s'] if len(tests) > 1 and tests[1]['s'] else '缺失',
        '硫测试3(%)': tests[2]['s'] if len(tests) > 2 and tests[2]['s'] else '缺失',
...
print(f'\n推断的原矿品位:')
print(f'  铜品位: {ore_cu}% (需要确认)')
print(f'  硫品位: {ore_s}% (需要确认)')
...
# 调整列顺序
cols = ['编号', '精矿重量(g)', '铜测试1(%)', '铜测试2(%)', '铜测试3(%)', '铜平均(%)',
        '硫测试1(%)', '硫测试2(%)', '硫测试3(%)', '硫平均(%)']
df = df[cols]
...
with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
    ...
    ore_info = pd.DataFrame({
        ...
        '数值': [ore_cu if ore_cu else '需确认', ore_s if ore_s else '需确认', 100]
    })
...
3. 确认后，我将计算所有试验组的回收率
'''.format(ore_cu if ore_cu else '?', ore_s if ore_s else '?'))
