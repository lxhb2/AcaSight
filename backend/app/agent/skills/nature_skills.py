...
async def paper_qa_handler(query: str, pdf_id: str) -> dict:
    ...
    pdf_id 可以是：
    - ChromaDB 中的 paper_id（整数）-> 向量检索
    - PDF 文件路径/proxy URL（非整数）-> 尝试从文件直接读取
    - 若都无法匹配 -> 返回错误提示
    """
    try:
        # 0. 清理被 LLM 污染的 pdf_id（移除空格/换行/编码碎片）
        import re
        pdf_id = re.sub(r'\s+', '', pdf_id)  # 移除所有空白字符
        # 修复常见的 LLM 编码错误：http%3F → ://
        pdf_id = pdf_id.replace('%3F//', '://').replace('%3f//', '://')
        
        # 1. 尝试 ChromaDB 向量检索（仅当 pdf_id 是整数）
        if pdf_id.isdigit():
            from app.services.vector_service import vector_service
            relevant_chunks = await vector_service.asearch(
                query, filter={"paper_id": int(pdf_id)}, top_k=5
            )
            if relevant_chunks:
                context_text = "\n\n---\n\n".join(
        ...
        # 2. ChromaDB 无结果 或 pdf_id 不是整数 -> 尝试直接从文件读取
        try:
            from app.services.pdf_service import pdf_service
            from urllib.parse import urlparse, parse_qs, unquote
            file_path = pdf_id
            if "proxy?url=" in file_path:
                parsed = urlparse(pdf_id)
                qs = parse_qs(parsed.query)
                raw = qs.get("url", [pdf_id])[0]
                file_path = unquote(raw)
            text = pdf_service.extract_for_reading(file_path, max_chars=8000)
            if text:
                system_msg = (
                    "你是一位学术文献阅读助手。\n\n"
                    f"{READER_RULES}\n\n"
                    "请基于给定的文献段落回答以下问题。严格基于原文，不编造。"
                )
                messages = [
                    {"role": "system", "content": system_msg},
                    {"role": "user", "content": f"文献内容：\n{text[:6000]}\n\n问题：{query}"}
            ...
            logger.warning("Direct file read failed", pdf_id=pdf_id[:100], error=str(fe))

        # 3. 都失败了
        return {
            "answer": (
                f"无法定位 PDF 文件（`{pdf_id[:80]}`...），可能原因：\n"
                "1. PDF 尚未导入向量库（ChromaDB）\n"
                "2. PDF 文件路径不可访问\n\n"
                "建议：直接粘贴论文标题+摘要让我分析，或在 PDF 阅读器中选中文字后右键提问。"
            ),
            "citations": []
        }
...
async def generate_figure_v2_handler(
    content: str, visual_intent: str, task_type: str = "plot",
    style_guide: str = "nature", max_critic_rounds: int = 3
) -> dict:
    """PaperBanana 6-Agent Pipeline 图表生成"""
    from app.services.paper_banana_service import generate_figure_pipeline
    try:
        ...
        logger.error("generate_figure_v2 failed", error=str(e))
        return {"error": str(e), "image_base64": "", "code": ""}
...
def register_nature_skills(registry: "SkillRegistry"):
    ...
    # ====== PaperBanana 图表生成 v2 (Phase 10) ======
    registry.register(SkillDefinition(
        name="generate_figure_v2",
        description="PaperBanana 6-Agent插图Pipeline: 数据/文本→描述→代码生成→沙箱执行→Critic评估→修正循环。支持plot(统计图)和diagram(方法图)，Nature/IEEE/Esevier风格指南。输出base64图像+代码+评估报告。",
        category=SkillCategory.FIGURE,
        parameters={
            "type": "object",
            "properties": {
                "content": {"type": "string", "description": "原始数据(plot)或方法文本(diagram)"},
                "visual_intent": {"type": "string", "description": "图表意图/标题"},
                "task_type": {"type": "string", "enum": ["plot", "diagram"], "description": "plot=统计图, diagram=方法图"},
                "style_guide": {"type": "string", "enum": ["nature", "ieee", "elsevier"], "description": "SCI期刊风格指南"},
                "max_critic_rounds": {"type": "integer", "description": "最大评估轮数(1-5)", "default": 3}
            },
            "required": ["content", "visual_intent", "task_type"]
        },
        handler=generate_figure_v2_handler,
    ))
