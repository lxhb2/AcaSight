...
async def run_agent_task(request: AgentTaskRequest):
    ...
    - event: answer      — 最终回答
    - event: heartbeat   — 活动心跳
    - event: interrupted — 被用户中断
    - event: error       — 错误
    - event: done        — 完成
    """
        ...
        try:
            ...
            async for step in core.run(
                ...
                if event_type == "answer":
                    ...
                    assistant_content = f"[已中断] {step.get('content', '')}"
...
async def interrupt_agent(request: AgentInterruptRequest):
    """中断正在运行的 Agent 任务"""
    try:
        core = _get_agent_core()
        await core.interrupt()
        return {"detail": "已请求中断 Agent 任务", "conversation_id": request.conversation_id}
    except Exception as e:
        logger.error("interrupt failed", error=str(e))
        return {"error": f"中断失败: {str(e)}"}
...
async def agent_status():
    ...
    core = _get_agent_core()
    return {
        "api_call_count": core.api_call_count,
        "tool_call_count": core.tool_call_count,
        "interrupt_requested": core._interrupt_requested,
        "skills_loaded": len(core.skill_registry._skills) if core.skill_registry else 0,
    }
