...
async def search_papers(
    ...
    sort_by: str = Query("hybrid", description="排序方式: hybrid(默认)/year/citations/relevance"),
):
    ...
    if sort_by == "hybrid":
        ...
        current_year = 2026
        for p in merged:
            citations = p.get("cited_by_count") or p.get("citation_count") or p.get("citations") or 0
            ...
            # 引用分数（对数归一化）
            import math
            citation_score = math.log1p(citations) / math.log1p(1000) if citations else 0
            # 年份分数（越新越高）
            year_score = max(0, (year - 2000)) / (current_year - 2000) if year and year >= 2000 else 0
            # 关键词相关度分数（基于标题匹配）
            title = (p.get("title") or "").lower()
            keywords = [kw.lower() for kw in q.split() if len(kw) > 1]
            keyword_hits = sum(1 for kw in keywords if kw in title)
            keyword_score = keyword_hits / max(len(keywords), 1)
            
            # 混合权重：引用40% + 年份35% + 关键词25%
            p["_score"] = round(citation_score * 0.4 + year_score * 0.35 + keyword_score * 0.25, 4)
        ...
        return {
            "query": q,
            "sort_by": "hybrid",
            "sort_weights": {"citations": 0.4, "year": 0.35, "keywords": 0.25},
            "total": len(merged),
            "results": merged,
        }
