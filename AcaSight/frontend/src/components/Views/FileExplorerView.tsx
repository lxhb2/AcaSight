...
export const FileExplorerView: React.FC = () => {
  ...
  const importFromZotero = useCallback(async () => {
    ...
    const papersToImport = zoteroItems.map((item: any) => ({
      title: item.title || '未命名文献',
      authors: (item.creators || []).map((c: any) => c.name || `${c.firstName || ''} ${c.lastName || ''}`.trim()).filter(Boolean),
      ...
      tags: (item.tags || []).map((t: any) => t.tag || t).filter(Boolean),
      keywords: (item.tags || []).map((t: any) => t.tag || t).filter(Boolean),
    }));
    try {
