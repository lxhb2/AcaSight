...
@router.get("/hash")
async def get_pdf_hash(url: str = Query(..., description="PDF URL")):
...
@router.get("/proxy")
async def proxy_pdf(url: str = Query(..., description="PDF URL to fetch")):
    import httpx
    try:
        async with httpx.AsyncClient(timeout=30, follow_redirects=True) as client:
            resp = await client.get(url)
...
async def extract_full_text(req: ExtractTextRequest):
    """从 URL 对应的 PDF 提取全文文本（后端 PyMuPDF，稳定可靠）
