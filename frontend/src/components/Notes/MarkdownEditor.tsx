...
type MilkdownCtx = Parameters<InstanceType<typeof Editor>['config']>[0] extends (ctx: infer C) => infer _R ? C : never;
...
import {
  FileDown, Save, Undo2, Redo2, BookOpen,
} from 'lucide-react';
import { useApp } from '@/contexts/AppContext';
import { formatApi, notesApi } from '@/services/api';
...
interface MilkEditorInnerProps {
  ...
  handleFormatExport: (fmt: 'docx' | 'latex' | 'pdf' | 'html') => void;
  handleGenerateBib: () => void;
...
const MilkEditorInner: React.FC<MilkEditorInnerProps> = ({
  ...
  showExportMenu, setShowExportMenu, handleFormatExport, handleGenerateBib,
  toast, activeTab, setActiveTab,
}) => {
  useEditor((root) => {
    return (Editor.make() as any)
      .config((ctx: MilkdownCtx) => {
        ctx.set(rootCtx, root);
        ctx.set(defaultValueCtx, content);
        ctx.get(listenerCtx).markdownUpdated((_doc: unknown, md: string) => {
          setContent(md);
        });
      })
      .use(nord)
        ...
        <div style={{ position: 'relative' }}>
          ...
          {showExportMenu && (
            <div style={{ position: 'absolute', right: 0, top: '100%', marginTop: 6, background: 'var(--glass-bg)', backdropFilter: 'blur(var(--glass-blur))', border: '1px solid var(--hairline)', borderRadius: 8, padding: 8, minWidth: 200, zIndex: 50, boxShadow: 'var(--glass-shadow)' }}
              ...
              {(['docx', 'html', 'latex', 'pdf'] as const).map(fmt => (
                <button key={fmt} onClick={() => { handleFormatExport(fmt); setShowExportMenu(false); }} disabled={exporting}
                  ...
                  <FileDown size={12} /> 导出 {fmt === 'docx' ? 'Word (.docx)' : fmt === 'html' ? 'HTML (.html)' : fmt === 'latex' ? 'LaTeX (.tex)' : 'PDF (.pdf)'}
                </button>
              ))}
              <div style={{ borderTop: '1px solid var(--hairline)', margin: '4px 0' }} />
              <button onClick={() => { handleGenerateBib(); setShowExportMenu(false); }} disabled={exporting}
                className="mk-export-item"
                onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--accent-bg-soft)'; }}
                onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; }}>
                <BookOpen size={12} /> 生成 BibTeX (.bib)
              </button>
            </div>
          )}
        </div>
...
export const MarkdownEditor: React.FC<MarkdownEditorProps> = ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data = await notesApi.save({ note_id: noteId || null, content, title });
      showToast('笔记已保存', 'success');
      onSave?.(data.id, content);
    } catch (e: unknown) { showToast('保存失败: ' + (e instanceof Error ? e.message : String(e)), 'error'); }
    finally { setSaving(false); }
  };

  const handleFormatExport = async (format: 'docx' | 'latex' | 'pdf' | 'html') => {
    setExporting(true);
    try {
      const title = content.split('\n').find(l => l.startsWith('#'))?.replace(/^#+\s*/, '') || 'document';
      const resp = await formatApi.exportDocument({
        markdown: content,
        title,
        format,
        csl_style: selectedCsl || undefined,
      });
      if (!resp.ok) { const err = await resp.json().catch(() => ({})); throw new Error(err.detail || `导出失败 (${resp.status})`); }
      const blob = await resp.blob();
      const ext = format === 'docx' ? 'docx' : format === 'latex' ? 'tex' : format === 'html' ? 'html' : 'pdf';
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url; a.download = `${title}.${ext}`; a.click();
      URL.revokeObjectURL(url);
      showToast(`${format.toUpperCase()} 已导出`, 'success');
    } catch (e: unknown) { showToast('导出失败: ' + (e instanceof Error ? e.message : String(e)), 'error'); }
  ...
  const handleGenerateBib = async () => {
    setExporting(true);
    try {
      // Extract cited works from content (simple heuristic: look for @cite patterns)
      const citeMatches = content.match(/@[\w-]+/g) || [];
      if (citeMatches.length === 0) {
        showToast('未检测到引用标记（@cite 格式），请先添加引用', 'error');
        return;
      }
      const papers = citeMatches.map(c => ({ title: c.replace('@', '') }));
      const resp = await formatApi.generateBib(papers);
      if (!resp.ok) { const err = await resp.json().catch(() => ({})); throw new Error(err.detail || 'BibTeX生成失败'); }
      const blob = await resp.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url; a.download = 'references.bib'; a.click();
      URL.revokeObjectURL(url);
      showToast('BibTeX 已生成', 'success');
    } catch (e: unknown) { showToast('BibTeX生成失败: ' + (e instanceof Error ? e.message : String(e)), 'error'); }
    finally { setExporting(false); }
  };
  ...
  return (
    <MilkdownProvider>
      <MilkEditorInner
        ...
        handleFormatExport={handleFormatExport} handleGenerateBib={handleGenerateBib} toast={toast}
